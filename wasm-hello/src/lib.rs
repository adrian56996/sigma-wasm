...
impl HelloState {
    ...
    /// Set a new gum
    fn set_fave_soccer_team(&mut self, team: String) {
        self.team = team;
    }
}
...
#[wasm_bindgen]
pub fn get_fave_soccer_team() -> String {
    let state = HELLO_STATE.lock().unwrap();
    state.get_fave_soccer_team()
}
...
/// **To extend**: You could add validation, length limits, or formatting here.
/// 
/// @param gum - The new socce team to set
#[wasm_bindgen]
pub fn set_fave_soccer_team(team): String) {
    let mut state = HELLO_STATE.lock().unwrap();
